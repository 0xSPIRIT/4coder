...
view_pos_from_xy(Application_Links *app, View_ID view, Vec2_f32 p){
    ...
    // @spirit: Hack to get the clicking of the cursor to work properly and not be one line down.
    f32 filebar_height = 0.f;
    {
        Face_ID face_id = get_face_id(app, buffer);
        Face_Metrics face_metrics = get_face_metrics(app, face_id);
        f32 line_height = face_metrics.line_height;
        
        filebar_height = line_height + 2.f;
    }
    
    region.y0 += filebar_height;
    region.y1 += filebar_height;
...
query_user_general(Application_Links *app, Query_Bar *bar, b32 force_number, String_Const_u8 init_string){
    ...
    for (;;){
        ...
        //
        // Check for Ctrl+V to paste
        //if (*insert_string.str == 'v' && 
        ...
        //{
            ...
            //if (count > 0){
                ...
                //if (paste_index != 0){
                    //*paste_index = 0;
                    //
                    //Scratch_Block scr(app);
                    //
                    //String_Const_u8 clipboard = push_clipboard_index(scr, 0, *paste_index);
                    //clipboard = string_replace(scr, clipboard,
                                               //string_u8_litexpr("\n"),
                                               //string_u8_litexpr(""));
                    //clipboard = string_replace(scr, clipboard,
                                               //string_u8_litexpr("\t"),
                    ...
                    //if (clipboard.size > 0) {
                        //String_u8 string = Su8(bar->string.str, bar->string.size, bar->string_capacity);
                        //string_append(&string, clipboard);
                        //bar->string.size = string.string.size;
                    //}
                //}
            //}
        ...
        if (in.event.kind == InputEventKind_KeyStroke &&
            ...
            if (is_unmodified_key(&in.event)) {
                ...
                while (bar->string.size &&
                       character_is_whitespace(bar->string.str[bar->string.size-1])) 
                {
                    bar->string.str[bar->string.size-1] = 0;
                    bar->string.size--;
                }
                
                while (bar->string.size &&
                       character_is_letter(bar->string.str[bar->string.size-1])) 
                {
                    bar->string.str[bar->string.size-1] = 0;
                    bar->string.size--;
                }                    
