...
function int vim_mode_normal();
function int vim_mode_visual();
...
default_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id,
    ...
    } else if (vim_mode_visual()) {
        spirit_draw_visual_mode_cursor(app, view_id, buffer, text_layout_id, cursor_roundness);
    } else {
        if (spirit_insert_cursor_type == SPIRIT_INSERT_CURSOR_BLOCK) {
            spirit_draw_cursor(app, view_id, is_active_view, buffer, text_layout_id, cursor_roundness, mark_thickness);
        } else if (spirit_insert_cursor_type == SPIRIT_INSERT_CURSOR_LINE) {
