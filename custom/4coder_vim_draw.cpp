	...
	if(!has_highlight_range){
		...
		if(is_active_view && vim_lister_view_id == 0){
			...
			if(ACTIVE_BLINK(vim_cursor_blink) && !vim_is_selecting_register){
				//if(vim_state.mode == VIM_Insert){ rect = rect_split_top_bottom_neg(rect, 2.f).b; }
				...
				draw_rectangle_fcolor(app, rect, roundness, fcolor_id(defcolor_cursor, cursor_sub_id));
...
vim_draw_whole_screen(Application_Links *app, Frame_Info frame_info){
	...
	draw_rectangle(app, rect_split_top_bottom_neg(region, 2*pad+line_height).b, 0.f, back_color);
	draw_rectangle_outline(app, rect_split_top_bottom_neg(region, 2*pad+line_height).b, 0.f, 2.f, 0xFF323232);

	Vec2_f32 bot_left = {region.x0 + 4.f, region.y1 - line_height - pad};
#if 0    
	...
	if(vim_use_bottom_cursor){
		...
		draw_string(app, face_id, bot_string, bot_left, finalize_color(defcolor_echo_area, 0));
	}
    #endif
